...
def main():
    ...
    subparser = parser.add_subparsers(
        dest='subcommand_name', metavar='<subcommand>')
    ...
    add_kubos_command = functools.partial(
        kubos_options.command.add_command, local_config, subparser, 'kubos')  # add our own implemented commands
    # add from the default yotta commands
    add_yotta_command = functools.partial(
        kubos_options.command.add_command, local_config, subparser, 'yotta')
    ...
    add_yotta_command(
        'list', 'list', 'List the dependencies of the current module, or the inherited targets of the current target.')
    add_yotta_command('outdated', 'outdated',
    ...
    add_yotta_command('licenses', 'licenses',
                      'List the licenses of the current module and its dependencies.')
    add_yotta_command('clean', 'clean',
                      'Remove files created by yotta and the build.')
    add_yotta_command('config', 'config',
                      'Display the target configuration info.')
    add_yotta_command('shrinkwrap', 'shrinkwrap',
                      'Create a yotta-shrinkwrap.json file to freeze dependency versions.')
    add_kubos_command('version', 'version',
                      'Display the current active version of the cli and KubOS source repo.')
    add_kubos_command(
        'use', 'use', 'Set a new version of the KubOS modules to build your projects against.')
    add_kubos_command('versions', 'versions',
    ...
    following_args = functools.reduce(
        lambda x, y: x + ['--'] + y, split_args[1:], [])[1:]
